...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm max-w-2xl animate-slide-up">
    ...
    <form method="post" action="{{ route('admin.licenses.store') }}" class="p-5 space-y-4" id="license-form">
        ...
        <div>
            ...
            <select id="license_type" name="license_type" required class="w-full px-3.5 py-2.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow">
                <option value="subscription">Subscription (time-limited)</option>
                <option value="lifetime">Lifetime (no expiration)</option>
            </select>
            <p class="text-xs text-slate-500 mt-1">Select <strong>Subscription</strong> for time-limited licenses (requires valid until date). Select <strong>Lifetime</strong> for one-time payment licenses that never expire.</p>
        </div>
        <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div id="valid_until_container">
                <label for="valid_until" class="block text-sm font-medium text-slate-700 mb-1.5">Valid until (required for subscription)</label>
                <div class="flex gap-2">
